...
class DiceClassifier:
    ...
    def get_hog(self):
        hog = cv2.HOGDescriptor(self.dims, self.blockSize, self.blockStride, self.cellSize, self.nbins, self.derivAperture, self.winSigma, self.histogramNormType, self.L2HysThreshold, self.gammaCorrection, self.nlevels)
        return hog
    ...
    def sliding_window(self,frame):
        ...
        window_size = 100
        true_dice = []
        for y in range(0,frame.shape[0]-step_size,step_size):
            for x in range(0,frame.shape[1]-step_size,step_size):
